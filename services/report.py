...
def _yearly_detail(plans: list) -> str:
    ...
    for p in plans:
        ...
        lines.append("")
        lines.append("| 年份 | 租金收入 | 税费 | 折旧 | 运营成本 | 财务成本 | 运营毛利 | 贷款余额 |")
        lines.append("|------|---------|------|------|----------|----------|----------|----------|")
        for y in p["yearly"]:
            lines.append(
                ...
                f"| {_fmt(y['operating_profit'])} | {_fmt(y.get('loan_balance',0))} |"
            )
        lines.append("")
    return "\n".join(lines)
...
def _ppt_outline(config: dict, inv: dict, plans: list) -> str:
    ...
    lines = [
        "\u0023\u0023 七、PPT大纲",
        "",
        "> 以下每页 Slide 均可直接作为演示文稿的单页内容。建议图表类型标注于页内。",
        "",
        "### Slide 1：封面",
        f"- {config['meta']['name']}",
